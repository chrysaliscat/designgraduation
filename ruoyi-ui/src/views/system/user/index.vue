    ...
    <el-row :gutter="20">
        ...
        <el-col :span="24">
            <el-tabs v-model="activeTab" @tab-click="handleTabClick" type="card" style="margin-bottom: 10px;">
                ...
                <el-tab-pane label="买家" name="agri_buyer"></el-tab-pane>
                <el-tab-pane label="卖家" name="agri_seller"></el-tab-pane>
            ...
            <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
              ...
              <el-table-column label="身份" align="center" v-if="columns.identity.visible" key="identity">
                <template slot-scope="scope">
                    <div v-if="scope.row.roles && scope.row.roles.length > 0">
                        <el-tag v-for="role in scope.row.roles" :key="role.roleId" style="margin-right: 2px;" :disable-transitions="true">{{ role.roleName }}</el-tag>
                    </div>
                    <el-tag v-else-if="activeTab !== 'all'" :disable-transitions="true">{{ getIdentityLabel(activeTab) }}</el-tag>
                    <span v-else>--</span>
                </template>
              </el-table-column>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="角色">
              <!-- Disable role selection if editing an existing user (userId exists) to prevent logic issues -->
              <el-select v-model="form.roleIds" multiple placeholder="请选择角色" :disabled="form.userId != undefined">
...
export default {
  ...
  data() {
    return {
      ...
      columns: {
        ...
        identity: { label: '身份', visible: true },
  ...
  methods: {
    ...
    handleTabClick(tab) {
      ...
      if (tab.name === 'all') {
        this.queryParams.params['roleKey'] = undefined;
      } else {
        this.queryParams.params['roleKey'] = tab.name;
      }
      this.handleQuery();
    },
    getIdentityLabel(key) {
        const map = {
            'agri_buyer': '买家',
            'agri_seller': '卖家',
            'admin': '管理员'
